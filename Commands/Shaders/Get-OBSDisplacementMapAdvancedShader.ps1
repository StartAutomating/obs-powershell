...
[Alias('Set-OBSDisplacementMapAdvancedShader','Add-OBSDisplacementMapAdvancedShader')]
param(
# Set the displacement_info of OBSDisplacementMapAdvancedShader
[Alias('displacement_info')]
[ComponentModel.DefaultBindingProperty('displacement_info')]
[String]
$DisplacementInfo,
# Set the displacement_x of OBSDisplacementMapAdvancedShader
[Alias('displacement_x')]
[ComponentModel.DefaultBindingProperty('displacement_x')]
[Single]
$DisplacementX,
# Set the displacement_y of OBSDisplacementMapAdvancedShader
[Alias('displacement_y')]
[ComponentModel.DefaultBindingProperty('displacement_y')]
[Single]
$DisplacementY,
# Set the displacement_curve of OBSDisplacementMapAdvancedShader
...
[String]
$BlurInfo,
# Set the blur_size of OBSDisplacementMapAdvancedShader
[Alias('blur_size')]
[ComponentModel.DefaultBindingProperty('blur_size')]
[Single]
$BlurSize,
# Set the blur_quality of OBSDisplacementMapAdvancedShader
[Alias('blur_quality')]
...
[Single]
$BlurDirections,
# Set the blur_angle of OBSDisplacementMapAdvancedShader
[Alias('blur_angle')]
[ComponentModel.DefaultBindingProperty('blur_angle')]
[Single]
$BlurAngle,
# Set the chromatic_aberration_info of OBSDisplacementMapAdvancedShader
...
[Single]
$ChromaticAberration,
# Set the colorize_info of OBSDisplacementMapAdvancedShader
[Alias('colorize_info')]
[ComponentModel.DefaultBindingProperty('colorize_info')]
[String]
$ColorizeInfo,
# Set the colorize_color of OBSDisplacementMapAdvancedShader
...
[String]
$FlagsInfo,
# Set the blue_affects_strength of OBSDisplacementMapAdvancedShader
[Alias('blue_affects_strength')]
[ComponentModel.DefaultBindingProperty('blue_affects_strength')]
[Management.Automation.SwitchParameter]
$BlueAffectsStrength,
# Set the blue_affects_colorize of OBSDisplacementMapAdvancedShader
[Alias('blue_affects_colorize')]
[ComponentModel.DefaultBindingProperty('blue_affects_colorize')]
[Management.Automation.SwitchParameter]
$BlueAffectsColorize,
# Set the blue_affects_blur of OBSDisplacementMapAdvancedShader
[Alias('blue_affects_blur')]
[ComponentModel.DefaultBindingProperty('blue_affects_blur')]
[Management.Automation.SwitchParameter]
$BlueAffectsBlur,
# Set the alpha_affects_strength of OBSDisplacementMapAdvancedShader
...
[Management.Automation.SwitchParameter]
$ApplyAlpha,
# Set the mask_layer of OBSDisplacementMapAdvancedShader
[Alias('mask_layer')]
...
process {
$shaderName = 'displacement_map_advanced'
$ShaderNoun = 'OBSDisplacementMapAdvancedShader'
if (-not $psBoundParameters['ShaderText']) {    
    $psBoundParameters['ShaderText'] = $ShaderText = '
uniform string displacement_info<
  string label = "Displacement";
  string widget_type = "info";
> = "Displaces the current image with the Mask Layer. Red channel is affected to X (horizontal) displacement and Green channel to Y (vertical). rgb(.5, .5, .5) is no displacement. You can choose the curve mapping to map values between -1 to 1 differently.";
...
uniform texture2d mask_layer <
  string label = "Mask Layer";
>;
...
float4 mainImage(VertData v_in) : TARGET
{
  ...
  float4 map_rgba = mask_layer.Sample(textureSampler, v_in.uv);
  ...
  if(chromatic_aberration) {
    float4 base_r = image.Sample(textureSampler, v_in.uv + displace_uv - chromatic_aberration * displace_uv);
    float4 base_g = image.Sample(textureSampler, v_in.uv + displace_uv);
    float4 base_b = image.Sample(textureSampler, v_in.uv + displace_uv + chromatic_aberration * displace_uv);

    base_rgba = float4(base_r.r, base_g.g, base_b.b, base_g.a);
  } else {
    base_rgba = image.Sample(textureSampler, v_in.uv + displace_uv);
  }
  ...
  if (blur_size > 0 && displace.a > 0) {
    ...
    [loop] for( float d=blurAngleRadians; d < Pi + blurAngleRadians; d+=Pi/blur_directions) {
      [loop] for(float i=1.0 / blur_quality; i <= 1.0; i += 1.0 / blur_quality) {
        ...
        if (chromatic_aberration) {
          float4 sc_r = image.Sample(textureSampler, v_in.uv + displace_uv - chromatic_aberration*displace_uv + float2(cos(d),sin(d)) * size * i / uv_size);
          float4 sc_g = image.Sample(textureSampler, v_in.uv + displace_uv + float2(cos(d),sin(d)) * size * i / uv_size);
          float4 sc_b = image.Sample(textureSampler, v_in.uv + displace_uv + chromatic_aberration*displace_uv + float2(cos(d),sin(d)) * size * i / uv_size);

          sc = float4(sc_r.r, sc_g.g, sc_b.b, sc_g.a);
        } else {
          sc = image.Sample(textureSampler, v_in.uv + displace_uv + float2(cos(d),sin(d)) * size * i / uv_size);
        }
  ...
  if (apply_alpha) {
    float4 background_rgba = image.Sample(textureSampler, v_in.uv);
